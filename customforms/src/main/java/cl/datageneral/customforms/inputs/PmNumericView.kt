...
import cl.datageneral.customforms.helpers.MinMaxInputFilter


/**
 * Created by Pablo Molina on 27-10-2020. s.pablo.molina@gmail.com
 */
class PmNumericView(val readOnly:Boolean, context: Context, attrs: AttributeSet?=null): PmView(context, attrs) {
    //var isExternalText = false
    var listener:MainListener? = null
    var inputLabel: InputNumericView? = null
        set(value) {
            field           = value
            viewId          = value!!.viewId
            ...
            editable.filters = arrayOf(MinMaxInputFilter(value.minValue, value.maxValue))
            editable.inputType =
...
//    }
    ...
    init {
        if (readOnly) {
            inflate(context, R.layout.ro_numeric_view, this)
        } else {
            inflate(context, R.layout.pm_numeric_view, this)
        }
